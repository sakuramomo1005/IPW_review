...
# Since the second reviewer would like to see a smaller outcome ICC, 
# we propose to run simulations for outcome ICC=0.01 but for k=25 only 
# (i.e. not for k=10 and not for k=50). This would be at all 4 values of ICCM.

# setwd("") ## set a dictionary

setwd('/Users/yaolanqiu/Documents/IPW/Editing 2019-3-16/data')
...
mypool <- function(mean0,sd0,num=5,J=50){
  ...
  df_obs <- df_com*((df_com+1)/(df_com+3))*(1-1/parenthesis) 
...
# 2.7 Small sample corrections
# calculat the robust variance by hand
# correct the standard deviation by usign KC, MD, FG methods 
...
for(k in c(25)){ # number of clusters 
  for(icc in c(0.01)){ # icc for data generation
    for(iccm in c(0, 0.1, 0.3, 0.5)){ # icc for missingness
      ...
      result=list(est_ind=est_ind,est_ex=est_ex,
                  ...
                  df_t=df_t,
                  df_adj_t_incorrect=df_adj_t_incorrect,
                  df_adj_t=df_adj_t)
...
for(k in c(25)){
  for(icc in c(0.01)){
    for(iccm in c(0, 0.1, 0.3, 0.5)){
      ...
      names <- paste('geem_addition2_',k,icc,iccm,'.RData',sep='')
      save(result,file=names)
    }
  }
}
...
setwd('/Users/yaolanqiu/Documents/IPW/Editing 2019-3-16/data/addition2')
...
for(k in c(25)){
  ...
  colnames(unind)=colnames(unex)=c('ICC','ICCM','TRUE','CRA-GEE','A-CRA-GEE','W-GEE','CW-GEE')
...
colnames(uncov_ind) <- colnames(uncov_ex) <- c('ICC','ICCM','TRUE','CRA-GEE',
                                               'A-CRA-GEE','W-GEE','CW-GEE','MMI-GEE')
...
## FINDAL TABLES 
table_ind <- cbind(BIAS_ind,Cov_ind[,3:7],STD_ind[,3:7],MCSD_ind[,3:7],uncov_ind[,3:7])
table_ex <- cbind(BIAS_ex,Cov_ex[,3:7],STD_ex[,3:7],MCSD_ex[,3:7],uncov_ex[,3:7]) 
